...
class InAppNotificationServiceTest {
    ...
    @Test
    void notificaUsuario15MinAntesDeFinalizarReserva() {
        ...
        assertDoesNotThrow(() -> {
            ...
            verify(notificacionSender, times(1)).enviarNotificacion(eq(reserva), contains("Faltan 15 minutos"));
        });
    }

    @Test
    void notificaUsuario5MinAntesDeFinalizarReserva() {
        LocalDateTime fin = LocalDateTime.now().plusMinutes(5);
        Reserva reserva = new Reserva(2L, "Sala B", fin, true, false, false);
        assertDoesNotThrow(() -> {
            inAppNotificationService.notificar5MinAntes(reserva);
            verify(notificacionSender, times(1)).enviarNotificacion(eq(reserva), contains("Faltan 5 minutos"));
        });
    }

    @Test
    void notificaUsuario10MinDespuesSiNoEntrega() {
        LocalDateTime fin = LocalDateTime.now().minusMinutes(10);
        Reserva reserva = new Reserva(3L, "Sala C", fin, true, false, false);
        assertDoesNotThrow(() -> {
            inAppNotificationService.notificar10MinDespues(reserva);
            try {
                verify(notificacionSender, times(1)).enviarNotificacion(eq(reserva), contains("10 minutos"));
            } catch (Exception e) {
                fail(e);
            }
        });
    ...
    @Test
    void notificacionIncluyeNombreYFechaYTiempoExtraSiAplica() {
        LocalDateTime fin = LocalDateTime.now().minusMinutes(7);
    ...
    @Test
    void reintentaYLogueaSiFallaEnvio() {
        LocalDateTime fin = LocalDateTime.now().plusMinutes(15);
    ...
    @Test
    void noNotificaSiReservaCanceladaOModificada() {
        LocalDateTime fin = LocalDateTime.now().plusMinutes(15);
    ...
    @Test
    void soloNotificaEnHorarioLaboral() {
        LocalDateTime ahora = LocalDateTime.now();
        Reserva reserva = new Reserva(8L, "Sala H", ahora, true, false, false);
        inAppNotificationService.notificarSoloEnHorarioLaboral(reserva);
        // Puede enviar o no según la hora, pero siempre debe loguear si no notifica
        verify(logger, atLeast(0)).info(anyString());
    }
