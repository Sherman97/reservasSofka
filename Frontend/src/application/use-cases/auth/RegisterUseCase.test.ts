...
describe('RegisterUseCase', () => {
    const mockUser = new User({ id: 'u1', email: 'new.user@sofka.com.co', name: 'New User', role: 'user' });
    ...
    it('debe registrar usuario con datos válidos', async () => {
        ...
        const result = await useCase.execute({ email: 'new.user@sofka.com.co', password: '123456', name: 'New User' });
    ...
    it('debe lanzar error si password está vacío', async () => {
        const repo = createMockRepo();
        const useCase = new RegisterUseCase(repo);
        await expect(useCase.execute({ email: 'test@sofka.com.co', password: '', name: 'Test' }))
            .rejects.toThrow(RegistrationError);
    });
    ...
    it('debe lanzar error si name está vacío', async () => {
        const repo = createMockRepo();
        const useCase = new RegisterUseCase(repo);
        await expect(useCase.execute({ email: 'test@sofka.com.co', password: '123456', name: '' }))
            .rejects.toThrow(RegistrationError);
    });
    ...
    it('debe lanzar error si email es inválido', async () => {
        const repo = createMockRepo();
        const useCase = new RegisterUseCase(repo);
        await expect(useCase.execute({ email: 'test@gmail.com', password: '123456', name: 'Test' }))
            .rejects.toThrow(RegistrationError);
    });
    ...
    it('debe lanzar error si password tiene menos de 6 caracteres', async () => {
        const repo = createMockRepo();
        const useCase = new RegisterUseCase(repo);
        await expect(useCase.execute({ email: 'test@sofka.com.co', password: '12345', name: 'Test' }))
            .rejects.toThrow(RegistrationError);
    });
    ...
    it('debe aceptar password de exactamente 6 caracteres', async () => {
        const repo = createMockRepo();
        const useCase = new RegisterUseCase(repo);
        const result = await useCase.execute({ email: 'test@sofka.com.co', password: '123456', name: 'Test' });
        expect(result).toBe(mockUser);
    });
});
