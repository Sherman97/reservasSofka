...
export const useUserReservations = (): UseUserReservationsReturn => {
    ...
    const filteredReservations = useMemo((): Reservation[] => {
        return reservations.filter(res => {
            ...
            let matchesTab = true;
            if (activeTab === 'upcoming') matchesTab = res.isUpcoming();
            else if (activeTab === 'past') matchesTab = res.isPast() && res.isActive();
    ...
    return {
        ...
        cancelReservation: handleCancelReservation, reload: loadReservations
    };
};
